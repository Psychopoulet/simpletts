...
const readOptionsToEspeakArgs = require(join(
  __dirname,
  "readOptionsToEspeakArgs.js"
));
const readOptionsToSAPIArgs = require(join(
  __dirname,
  "readOptionsToSAPIArgs.js"
));
...
module.exports = class {
  constructor(scriptsFolderPath) {
    LISTVOICES = scriptsFolderPath
      ? resolve(scriptsFolderPath, "listvoices.vbs")
      : join(__dirname, "..", "batchs", "listvoices.vbs");
    PLAYTEXT = scriptsFolderPath
      ? resolve(scriptsFolderPath, "playtext.vbs")
      : join(__dirname, "..", "batchs", "playtext.vbs");
    ...
    return new Promise((resolve, reject) => {
      ...
      voices.forEach(_voice => {
        const voice = {
          ...
          name: IS_SAPI
            ? _voice.Name.trim().toLowerCase()
            : _voice.VoiceName.trim().toLowerCase()
        };
  ...
  read(_options) {
    ...
    if ("undefined" === typeof _options) {
      ...
      const options =
        "string" === typeof _options ? { text: _options } : _options;
        ...
        this._readPromise = Promise.resolve()
          .then(() => {
            return this.defaultVoice
              ? Promise.resolve(this.defaultVoice)
          ...
          .then(() => {
            if (
              "object" !== typeof options.voice &&
              "string" !== typeof options.voice
            ) {
              options.voice = this.defaultVoice;
            }

            options.volume =
              "undefined" === typeof options.volume
                ? 100
                : Math.round(options.volume);
            options.volume = 0 > options.volume ? 0 : options.volume;
            options.volume = 100 < options.volume ? 100 : options.volume;

            options.speed =
              "undefined" === typeof options.speed
                ? 50
                : Math.round(options.speed);
                  ...
                  .then(args => {
                    return new Promise((resolve, reject) => {
                      this._reader = IS_SAPI
                        ? spawn(
                            "cscript",
                            CSCRIPT_ARGS.concat([PLAYTEXT]).concat(args)
                          )
                        : spawn("espeak", args);
