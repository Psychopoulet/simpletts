...
module.exports = class {
				...
				return Promise.resolve().then(() => {
					...
					return this._forceStop ? Promise.resolve(options) : Promise.resolve().then(() => {
						...
						return new Promise((resolve, reject) => {
							...
							let err = "";
							this.reader.stderr.on("data", (data) => {
								err += "string" === typeof data ? data : data.toString("ascii");
							});

							this.reader.on("close", (code) => {
								return code ? reject(new Error(err)) : resolve();
							});

						});

					}).then(() => {
						this.reader = null; return Promise.resolve(options);
					}).catch((err) => {
						this.reader = null; return Promise.reject(err);
					});

				}).then((data) => {
					this._forceStop = false; return Promise.resolve(data);
				});
	...
	stopReading () {
		...
		return !this.isReading() ? Promise.resolve().then(() => {
			this._forceStop = true; return Promise.resolve();
		}) : new Promise((resolve, reject) => {

			try {
				this.reader.kill(); resolve();
			}
			catch (e) {
				reject(e);
			}
